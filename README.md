...
- **按年级注册**：注册时选择当前年级（大一~大四），系统自动推算毕业注销日
- **毕业自动注销**：毕业日（入学年 + 4 年的 6 月 30 日，学年以 7 月 1 日分界）当天起，每日自动删除毕业用户的账户及全部数据（课程 / 任务 / 日志 / 密钥 / 设置），管理员账户豁免
- 管理员：用户列表（年级、毕业注销日、课程 / 任务 / 密钥统计）、昵称 / 角色修改、重置密码、停用（JWT 与 API Key 即时失效）、删除（级联清理）
- 防自锁：管理员不能停用 / 降级 / 删除自己
